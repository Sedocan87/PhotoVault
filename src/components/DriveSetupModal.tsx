...
const DriveSetupModal: React.FC<DriveSetupModalProps> = ({
  isOpen,
  onClose,
  onConfigSaved,
}) => {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Select a primary drive for your photo library and a backup drive for
            synchronization.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              onClick={() => selectDirectory(setPrimaryDrive)}
              className="flex-grow"
            >
              Select Primary Drive
            </Button>
            <p className="text-sm text-muted-foreground truncate">
              {primaryDrive || "No directory selected"}
            </p>
          </div>
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              onClick={() => selectDirectory(setBackupDrive)}
              className="flex-grow"
            >
              Select Backup Drive
            </Button>
            <p className="text-sm text-muted-foreground truncate">
              {backupDrive || "No directory selected"}
            </p>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={handleSave}
            disabled={isSaving || !primaryDrive || !backupDrive}
          >
